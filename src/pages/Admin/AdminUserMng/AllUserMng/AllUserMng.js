            ...
            <div>
                {row.status === "Available" && (
                    <button className={cx("ban_btn")} onClick={() => {
                        axios.post("/api/v1/admin/action/user/" + row.id + "?action=BAN")
                ...
                {row.status === "Banned" && (
                    <button
                        ...
                        onClick={() => {
                            axios.post("/api/v1/admin/action/user/" + row.id + "?action=RECOVER")
    ...
    useEffect(() => {
        axios.get("/api/v1/admin/management/user")
            .then(res => {
                ...
                setRecords(mappedUser)
                setSearch(mappedUser);
            })
