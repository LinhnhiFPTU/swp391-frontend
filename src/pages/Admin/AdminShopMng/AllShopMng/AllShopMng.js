            ...
            <div>
                {row.status === "Available" && (
                    <button className={cx("ban_btn")} onClick={() => {
                        axios.post("/api/v1/admin/action/shop/" + row.id + "?action=BAN")
                ...
                {row.status === "Banned" && (
                    <button
                        ...
                        onClick={() => {
                            axios.post("/api/v1/admin/action/shop/" + row.id + "?action=RECOVER")
                                .then(res => window.location.reload())
                                .catch(e => console.log(e))
                        }}
                    >
...
function AllShopMng() {
    ...
    useEffect(() => {
        axios.get("/api/v1/admin/management/shop")
            .then(res => {
                ...
                let mappedShop = res.data.map(item => {
                    let mapped = {
                        ...
                        package: item.shopPackages[item.shopPackages.length - 1].shopPlan.plan + " - " + item.shopPackages[item.shopPackages.length - 1].shopPlan.duration + " days",
