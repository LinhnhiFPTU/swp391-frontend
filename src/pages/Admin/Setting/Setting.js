...
function Setting() {
  ...
  useEffect(() => {
    axios.get("/api/v1/admin/management/setting")
    .then(res => {
  ...
  {
    e.preventDefault()
    axios.post("/api/v1/admin/management/setting/update", settingsList)
    .then(res => {
                ...
                {settingsList.map((item) => (
                  <div className={cx("setting-item")} key={item.id}>
                    ...
                    <div className={cx("edit-setting")}>
                      <input type="number" className={cx("input")} value={item.value} onChange={(e) => handleChange(e, item)}/>
                    </div>
                  </div>
                ))}
              </div>
