...
function EditProduct({ product, setShowEdit }) {
  const fileInputImageRef = useRef();
  const fileInputVideoRef = useRef();
  const videoPreview = useRef();
  const [quantityImage, setQuantityImage] = useState(product.listImage.length);
  const [listImageEdit, setListImageEdit] = useState(product.listImage);
  const [video, setVideo] = useState(product.video);
  const [videoDuration, setVideoDuration] = useState("00:00");
  const [showDeleteVideo, setShowDeleteVideo] = useState(false);
  const [videoError, setVideoError] = useState("");
  const [countInput, setCountInput] = useState(product.productName.length);
  const [inputNameProduct, setInputNameProduct] = useState(product.productName);
  ...
  const handlePreviewImage = (e) => {
    ...
    setListImageEdit((prev) => [...prev, file.preview]);
